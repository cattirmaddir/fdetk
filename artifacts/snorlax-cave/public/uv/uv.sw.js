...
class UVServiceWorker extends EventEmitter {   
    ...
    getBarerResponse(response) {
        const headers = {};
        const rawHeader = response.headers.get('x-bare-headers');
        const statusHeader = response.headers.get('x-bare-status');
        const parsedStatus = Number(statusHeader);
        ...
        return {
            headers,
            status,
            statusText: response.headers.get('x-bare-status-text') || response.statusText || 'Bad Gateway',
